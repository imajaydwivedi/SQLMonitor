...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'(dba) ReSetup-PerfmonDataCollector', 
		@enabled=1, 
...
IF exists (select * from msdb..sysjobs_view j where j.name = '(dba) ReSetup-PerfmonDataCollector')
	exec msdb.dbo.sp_start_job @job_name = '(dba) ReSetup-PerfmonDataCollector';

WAITFOR DELAY '00:10:00';

IF exists (select * from msdb..sysjobs_view j where j.name = '(dba) ReSetup-PerfmonDataCollector')
	exec msdb.dbo.sp_delete_job @job_name = '(dba) ReSetup-PerfmonDataCollector';
GO
