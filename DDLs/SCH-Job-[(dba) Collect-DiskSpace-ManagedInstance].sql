...
IF EXISTS (SELECT * FROM msdb.dbo.sysjobs_view WHERE name = N'(dba) Collect-DiskSpace')
	EXEC msdb.dbo.sp_delete_job @job_name='(dba) Collect-DiskSpace', @delete_unused_schedule=1
GO
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'dbo.usp_collect_disk_space', 
		@step_id=1, 
		...
		@command=N'sqlcmd -C -E -b -S localhost -H "(dba) Collect-DiskSpace" -d DBA -Q "EXEC dbo.usp_collect_disk_space"', 
		@flags=40
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC msdb.dbo.sp_start_job @job_name='(dba) Collect-DiskSpace'
go
