...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'dbo.alert_history_all_servers', 
		...
		@database_name=N'DBA', 
		@command=N'SET CONCAT_NULL_YIELDS_NULL ON; EXEC dbo.usp_wrapper_GetAllServerCollectedData @step_name = ''dbo.alert_history_all_servers'', @has_staging_table = 0, @truncate_table = 0, @verbose = 0;', 
		@flags=40
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
